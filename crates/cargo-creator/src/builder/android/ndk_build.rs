...
impl Ndk {
    ...
    pub fn toolchain_bin(&self, bin: &str, target: AndroidTarget) -> Result<PathBuf, NdkError> {
        #[cfg(target_os = "windows")]
        let ext = ".exe";
        #[cfg(not(target_os = "windows"))]
        let ext = "";
    ...
    pub fn sysroot_lib_dir(&self, target: AndroidTarget) -> Result<PathBuf, NdkError> {
    ...
    pub fn detect_abi(&self) -> Result<AndroidTarget, NdkError> {
        let stdout = self
            .platform_tool("adb")?
            .arg("shell")
            .arg("getprop")
