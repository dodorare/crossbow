...
fn test_cargo_metadata() {
    ...
    config
        .status_message("Reading metadata", package_name)
        .unwrap();

    let android_manifest =
        AndroidBuildCommand::get_android_manifest(&context, AndroidStrategy::GradleApk).unwrap();
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.crossbow.example" android:versionCode="1" android:versionName="0.1.0">
  <application android:hasCode="true" android:label="Crossbow" android:theme="@android:style/Theme.DeviceDefault.NoActionBar.Fullscreen">
    <activity android:name="com.crossbow.example.MainActivity" android:configChanges="orientation|keyboardHidden|screenSize" exported="true" android:resizeableActivity="true">
      <intent-filter>
