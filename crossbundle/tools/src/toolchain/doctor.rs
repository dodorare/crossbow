...
use android::{
    bundletool_check, compatible_child, executable_version_from_metadata,
    project_rust_targets_check, versioned_executable_check,
};
#[cfg(feature = "apple")]
mod apple;
mod project;
...
fn rust_sysroot(environment: &Environment, project_dir: &Path) -> Option<PathBuf> {
    let rustup_home = environment
        ...
        .get("RUSTUP_HOME")
        .map(PathBuf::from)
        .or_else(|| home::home_dir().map(|home| home.join(".rustup")))?;
